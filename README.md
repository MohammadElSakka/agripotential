# AgriPotential
## A Novel Multi-Sepctral and Multi-Temporal Remote Sensing Dataset for Agricultural Potential Prediction
This repository contains Python utility functions and classes designed to use the AgriPotential Dataset. 

Please refer to the tutorial below for more instructions and information.

## Tutorial 
https://colab.research.google.com/drive/1Ys_z7qBY1pa8iY7PMaLzDsTuiFaJDhHu

## Competition link
https://www.codabench.org/competitions/12055/

## Citation
If you find this work helpful please cite the following:
```
@article{sakka2025agripotential,
title={AgriPotential: A Novel Multi-Spectral and Multi-Temporal Remote Sensing Dataset for Agricultural Potentials},
author={Sakka, Mohammad El and De Pourtales, Caroline and Chaari, Lotfi and Mothe, Josiane},
  journal={arXiv preprint arXiv:2506.11740},
  year={2025}
}
```
